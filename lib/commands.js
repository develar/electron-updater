...
// attempt to update this app
function update(appDir, logger, callback) {
  //assertions
  ...
  //checking to see if an update needs to be made
	check(appDir, logger, function (err, results) {
		if(err) return callback(err)
		if(!results) return callback()

		logger.log('Updating...')
		updater.update(results, logger, callback)
	})
}
  ...
  //assertions
	...
	context.load(appDir, function (err, ctx) {
		...
		function (err, results) {
			if(err) return callback(err);
      // filter out what doesn't need updates
			var app = results[0];
        ...
        //at least one item needs an update
...
function fullUpdate(publisher, appName, logger, callback) {
  // callout to the copy process
  // copy the executable into the update directory
	copier.copy(process.execPath, publisher, appName, function (err, tmpExecPath) {
		if(err) return callback(err);
    if (require('os').platform() == 'darwin'){
      tmpExecPath = tmpExecPath.replace(" ", "\\ ") + "/Contents/MacOS/Electron";
    }
    // fetch update directory
    ...
    console.log(tmpExecPath + " " + updateDir + " --electron-update=" + encodedArgs);
    // fire off a process running out temp executable, requesting that electron update
...
function start(appDir, logger, callback) {
	...
	expect('string', 'object', 'function', arguments, function (values) {
		appDir = values[0] || path.dirname(process.mainModule.filename)
		logger = values[1] || new Logger(appDir, Logger.appendToFile, true)
				...
				if(valid) {
					...
					check(appDir, logger, function (err, result) {
						...
						if(result && (result.app || result.dependencies.length)) {
							...
							updater.update(result, logger, function (err) {
								if(err) return callback(err)
								that.emit('updateAvailable')
								callback()
							})
