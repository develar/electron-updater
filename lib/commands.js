...
function notify(appDir, logger) {
	expect('string', 'object', arguments, function (values) {
		appDir = values[0] || path.dirname(process.mainModule.filename)
		logger = values[1] || Logger.default
	})
	return new Notifier(appDir, logger, check);
}
...
function isValid(appDir, logger, callback) {
	...
	context.load(appDir, function (err, ctx) {
		if(err) return callback(err)
		async.map([
			{ context: ctx, kind: 'dependencies', logger: logger },
			{ context: ctx, kind: 'plugins', logger: logger }
		],
		exists.check,
		function (err, results) {
			if(err) return callback(err)
			var r1 = results[0]
			var r2 = results[1]
			if (!r1) logger.log('Dependencies are not valid.');
			if (!r2) logger.log('Plugins are not valid.');
			var valid = r1 && r2
			callback(null, valid)
		})
	})
...
function start(appDir, logger, callback) {
	...
	context.load(appDir, function (err, ctx) {
		...
		if(ctx.updatePending) {
			...
			logger.log("Update is pending, exiting...");
...
ElectronCommands.prototype.notify = notify
